...
export const RadarRowComponent = ({ rowData }) => {
    ...
    const handleIsPublishedClick = (event) => {
       ...
       if(event.target.checked == true){
        ...
        if(shouldProcess==true){
           ...
           repo.publishRadar(authenticatedUser.currentlyViewedSubscriptionId, rowData.id, event.target.checked, handleRadarChangeResponse);
        }
    }
    ...
    const handleRadarChangeResponse = (wasSuccessful) => {
        if(wasSuccessful==true){
            let repo = new AccountAdminRepository();
            repo.getRadars(authenticatedUser.currentlyViewedSubscriptionId, handleGetUserRadarResponse);
        }
    }
    ...
    const handleIsLockedClick = (event) => {
        ...
        let repo = new AccountAdminRepository();
        repo.lockRadar(authenticatedUser.currentlyViewedSubscriptionId, rowData.id, event.target.checked, handleRadarChangeResponse);
    }

    const handleDeleteClick = (event) => {
        let repo = new AccountAdminRepository();
        repo.deleteRadar(authenticatedUser.currentlyViewedSubscriptionId, rowData.id, handleRadarChangeResponse);
    }
